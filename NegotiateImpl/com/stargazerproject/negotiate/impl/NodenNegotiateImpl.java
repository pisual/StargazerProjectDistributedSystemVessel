 ...
 *              工作流程：
 *                          状态                                                              
 *                              启动状态 -> 步骤 1：等待状态（静默成员模式，允许被邀请，不能邀请别人
 *                                        		 -> 静默成员模式中如果被邀请成功，进入步骤5
 *                            			  步骤 2: 如果此时没有被邀请，准备进入寻索模式，成功将进行下一步，失败（正在被邀请或已经组建成功）返回步骤1
 *                            			  步骤 3: 寻索状态，根据布式分组算法在列表中获取想要邀请的成员，获取成功进入下一步骤，失败的话根据最大尝试数目进行再次尝试，
 *                                                        如果成功，进入下一步骤，失败的话，返回 步骤 1。
 *                                         步骤 4: 组Leader状态
 *                                         步骤 5: 组成员状态
 *                                         
 *                      
